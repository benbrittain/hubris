...
impl ToTokens for SpiConfig {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let device_code = self.devices.values().map(|dev| {
            ...
            let cs_port = dev.cs_port;
            let cs_pin = dev.cs_pin;
            let freq: syn::Ident =
...
impl ToTokens for SpiMuxOptionConfig {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let miso_port = &self.miso_port;
        let mosi_port = &self.mosi_port;
        let sck_port = &self.sck_port;
        tokens.append_all(quote::quote! {
            SpiMuxOption {
                miso_port: Port(#miso_port),
                miso_pin: Pin(#miso_pin),
                mosi_port: Port(#mosi_port),
                mosi_pin: Pin(#mosi_pin),
                sck_port: Port(#sck_port),
                sck_pin: Pin(#sck_pin),
            }
        });
    }
